...
class MIL_SB(nn.Module):
    def __init__(self, instance_loss = cross_entropy_with_probs,  k=20, classes=2,  input_dim=2048, hidden_dim1=512, hidden_dim2=256, dropout_rate=0.25, k_selection = "shuffle"):
        super().__init__()
        self.instance_loss = cross_entropy_with_probs
        self.k = k
    ...
    def instance_evaluation(self, A, h, classifier):
        ...
        if self.k_selection == "middle_wrong":
           ...
           return instance_loss, torch.topk(logits, 1, dim=1)[1].squeeze(1), all_targets
            ...
            logits = classifier(all_instances)
            instance_loss = self.instance_loss(logits, all_targets_probs)
            return instance_loss, torch.topk(logits, 1, dim=1)[1].squeeze(1), all_targets
    ...
    def instance_evaluation_out(self, A, h,  classifier):
        ...
        #print(f"A {A.shape}")
        #print(f"h {h.shape}")
        top_k = torch.index_select(h, dim=0, index=torch.topk(A, self.k)[1])

        top_targets = torch.full((self.k, ), 0, device=device).long()
        # print(f"top_k {top_k.shape}")
